...
- `backend/` — the FastAPI side of the homelab pipeline, built as one image (`acoes-backend`) with **three entrypoints**, chosen per-workload via `command:`: `processor.py` (write path — `POST /candles`, plus `GET /watchlist` for the scraper), `api.py` (read path — `GET /candles`, `GET`/`PUT /watchlist`, `GET /status`), and `migrate.py` (applies `schema.sql`, run as an ArgoCD PreSync hook). `db.py`, `models.py` and `auth.py` are shared. Keeping the migration in the same image as the app is what stops schema and code drifting apart between deploys.
